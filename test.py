...
flip_threshold = 0.04 #Threshold below which an <Sz> sign flip is not flagged as being caused by the SVD
...
def main():
    ...
    DENS1.Gamma_mat[0,2,0,0] = 1/4 
    DENS1.Gamma_mat[0,3,0,0] = 1/4 
    
    DENS1.Gamma_mat[1,0,0,0] = 1/2 
    DENS1.Gamma_mat[1,1,0,0] = 1/2 
    DENS1.Gamma_mat[1,2,0,0] = 1/2 
    DENS1.Gamma_mat[1,3,0,0] = 1/2 
    
    DENS1.Gamma_mat[2,0,0,0] = 1/4 
    ...
    Sx_L = np.kron(Sx, np.eye(d))
    Sx_R = np.kron(np.eye(d), Sx)
    Sy_L = np.kron(Sy, np.eye(d))
    Sy_R = np.kron(np.eye(d), Sy)
    Ham_Dens = np.kron(Sx_L, Sx_L) + np.kron(Sy_L, Sy_L) - np.transpose(np.kron(Sx_R, Sx_R)) - np.transpose(np.kron(Sy_R, Sy_R))
    Ham_Dens *= -t_hopping/2        

    
    OPP = expm(-1j*test_dt*Ham)
    OPP_Dens = expm(-1j*test_dt*Ham_Dens)
    ...
    for t in range(test_steps):
        ...
        #    temp = DENS2.expval(np.kron(Sz, np.eye(d)), 1)
    ...
    plt.ylabel("<Sz>")
    plt.ylim((-1.05, 1.05))
    plt.title("<Sz> of DENS1 over time")
    plt.legend(bbox_to_anchor=(1.04, 0.5), loc="center left", borderaxespad=0)
    plt.grid()
    plt.show()
